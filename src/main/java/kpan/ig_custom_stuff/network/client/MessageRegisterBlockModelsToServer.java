...
public class MessageRegisterBlockModelsToServer extends MessageBase {
	//デフォルトコンストラクタは必須
	public MessageRegisterBlockModelsToServer() { }

	private Map<BlockModelId, BlockModelEntryBase> modelId2Entry;

	public MessageRegisterBlockModelsToServer(Map<BlockModelId, BlockModelEntryBase> modelId2Entry) {
	...
	public void doAction(MessageContext ctx) {
		...
		for (Entry<BlockModelId, BlockModelEntryBase> entry : modelId2Entry.entrySet()) {
			try {
				if (Server.INSTANCE.addBlockModel(entry.getKey(), entry.getValue()))
					...
					TextComponentTranslation component = new TextComponentTranslation("registry_message.block_model.error.already_exists", entry.getKey());
					MessageUtil.sendToServerAndAllPlayers(server, component);
				}
			} catch (IOException e) {
				ModMain.LOGGER.error("Failed to save a block model file", e);
				TextComponentTranslation component = new TextComponentTranslation("registry_message.block_model.register.failed", entry.getKey());
				MessageUtil.sendToServerAndAllPlayers(server, component);
			}
		}
		...
		MyPacketHandler.sendToAllPlayers(new MessageRegisterBlockModelsToClient(succeeded));
		ITextComponent component;
		if (succeeded.size() == 1) {
			component = new TextComponentTranslation("registry_message.block_model.register.success", succeeded.keySet().iterator().next(), sender.getDisplayName());
		} else {
			component = new TextComponentTranslation("registry_message.block_model.register.success.multiple", succeeded.size(), sender.getDisplayName());
		}
		MessageUtil.sendToServerAndAllPlayers(server, component);
	}
