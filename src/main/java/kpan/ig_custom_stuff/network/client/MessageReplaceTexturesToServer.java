...
public class MessageReplaceTexturesToServer extends MessageBase {
	...
	public MessageReplaceTexturesToServer(Map<ItemTextureId, Tuple2<byte[], @Nullable TextureAnimationEntry>> itemTextures, Map<BlockTextureId, Tuple2<byte[], @Nullable TextureAnimationEntry>> blockTextures) {
	...
	public void doAction(MessageContext ctx) {
		...
		MyPacketHandler.sendToAllPlayers(new MessageReplaceTexturesToClient(succeededItem, succeededBlock));
		ITextComponent component;
		if (succeededItem.size() + succeededBlock.size() == 1) {
			if (succeededItem.size() == 1)
				component = new TextComponentTranslation("registry_message.texture.update.success", succeededItem.keySet().iterator().next(), sender.getDisplayName());
			else
				component = new TextComponentTranslation("registry_message.texture.update.success", succeededBlock.keySet().iterator().next(), sender.getDisplayName());
		} else {
			component = new TextComponentTranslation("registry_message.texture.update.success.multiple", succeededItem.size() + succeededBlock.size(), sender.getDisplayName());
		}
		MessageUtil.sendToServerAndAllPlayers(server, component);
